...
public class Line3D {
	...
	private Line line;
	private double[] Xparam = new double[2], Yparam = new double[2], Zparam = new double[2], Xsymm = new double[2], Ysymm = new double[2], Zsymm = new double[2], tmpIntersect = new double[3];
...
//			Log.e("inhere", amin + " ++ "+ amin2);
	...
	public boolean isParallelTo(Line3D ln) {
		...
		if ((lndir[0]/(ptB.getX()-ptA.getX())) == (lndir[1]/(ptB.getY()-ptA.getY())) && ((lndir[1]/(ptB.getY()-ptA.getY())) == (lndir[2]/(ptB.getZ()-ptA.getZ()))))
			return true;
		else
			return false;
	}
